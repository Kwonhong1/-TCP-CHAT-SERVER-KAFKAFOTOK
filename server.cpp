...
#include <atomic>
#include <queue>
#include <cstring> // std::memcpy, std::strcpy 등을 위해 추가
...
public:
    ...
    void DispatchMessage(std::shared_ptr<ChatSession> session, const PacketHeader& header, const char* data, size_t size)
    {
        ...
        {
            std::cout << "Unknown message type: " << static_cast<uint16_t>(header.message_type) << std::endl;
        }
    }
    ...
    void Do_read()
    {
        ...
        socket_.async_read_some(
            ...
            [this, self](boost::system::error_code ec, std::size_t length)
            {
                if (!ec)
                {
                    packet_buffer_.WriteData(read_buffer_.data(), length); // length() 오타 수정
...
public:
    ...
    {
        std::lock_guard<std::mutex> lock(users_mutex_);
        if (users_.size() >= max_users_ || users_.find(user->GetId()) != users_.end())
            return false;
...
public:
    ...
    void OnSessionDisconnected(std::shared_ptr<ChatSession> session)
    {
        ...
        auto user = user_manager_.GetUser(user_id); // 오타 수정
        if (user)
        {
            user->SetOnline(false); // 오타 수정
            ...
            std::cout << "[System] Session disconnected." << std::endl;
        }
    }
    ...
    void ProcessLogin(std::shared_ptr<ChatSession> session, const LoginRequest& request)
    {
        ...
        std::memset(&response, 0, sizeof(LoginResponse));
        ...
        else
        {
            ...
            std::strncpy(response.error_message, "Invalid username or password.", sizeof(response.error_message) - 1);
        }
    ...
    void CreateRoom(uint32_t room_id, const std::string& name, uint32_t max_users)
    {
        ...
        std::cout << "[System] Room Created: " << name << " (ID: " << room_id << ")" << std::endl;
    }

    std::shared_ptr<ChatRoom> GetRoom(uint32_t room_id)
    {
        std::lock_guard<std::mutex> lock(rooms_mutex_);
        auto it = rooms_.find(room_id); // 오타 수정
...
void ChatSession::ProcessPacket(const char* data, size_t size)
{
    ...
    server_.GetDispatcher().DispatchMessage(shared_from_this(), header, data, size); // 오타 수정
}
