...
public:
    void HandleMessage(std::shared_ptr<ChatClient> client, const char* data, size_t size) override {
        ...
        std::string sender = std::to_string(msg.header.user_id);
...
public:
    void HandleMessage(std::shared_ptr<ChatClient> client, const char* data, size_t size) override {
        ...
        if (res.success) {
            ...
            client->SetRoomCreatedFlag(true); // 플래그 ON
...
int main()
{
    ...
    while (client->IsConnected() && is_running)
    {
        ...
        else if (choice == 2)
        {
            ...
            // 서버가 생성과 동시에 유저를 HOST로 입장시키고
            // created_room_id / owner_id를 함께 반환한다.
            // 따라서 별도의 JOIN_ROOM 왕복은 필요 없다.
            bool created = false;
            for (int i = 0; i < 500 && client->IsConnected(); ++i)
            {
                if (client->GetRoomCreatedFlag())
                {
                    created = true;
                    break;
                }
            ...
            if (!created)
            {
                std::cout << "[System] Room creation timed out or failed." << std::endl;
                continue;
            }
            ...
            std::cout << "\n==========================================" << std::endl;
            std::cout << " Room Created & Entered! Room ID: [" << created_room_id << "]" << std::endl;
            std::cout << " You are the Room Leader. (/kick, /pass commands available)" << std::endl;
    ...
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            ...
            while (client->IsConnected())
            {
                ...
                if (line == "/quit" || line == "/exit")
                {
                    ...
                    leave_req.room_id = created_room_id;

                    client->Send(&leave_req, sizeof(LeaveRoomRequest));
                    client->SetLastRoomId(0);
                    client->SetCurrentRoomOwnerId(0);
                    client->SetRoomCreatedFlag(false); // 플래그 초기화
                    std::cout << "[System] Left Room " << created_room_id << ", returning to Lobby menu..." << std::endl;
                    break;
                }
                ...
                if (line.rfind("/kick ", 0) == 0)
                {
                    ...
                    KickUserRequest kick_req{};
                    kick_req.header.packet_size = sizeof(KickUserRequest);
                    kick_req.header.message_type = MessageType::KICK_USER_REQUEST;
                    kick_req.header.user_id = client->GetUserId();
                    kick_req.room_id = created_room_id;
                    kick_req.target_user_id = target_user_id;

                    client->Send(&kick_req, sizeof(KickUserRequest));
                    continue;
                }
                ...
                if (line.rfind("/pass ", 0) == 0)
                {
                    ...
                    TransferMasterRequest pass_req{};
                    pass_req.header.packet_size = sizeof(TransferMasterRequest);
                    pass_req.header.message_type = MessageType::TRANSFER_MASTER_REQUEST;
                    pass_req.header.user_id = client->GetUserId();
                    pass_req.room_id = created_room_id;
                    pass_req.target_user_id = target_user_id;

                    client->Send(&pass_req, sizeof(TransferMasterRequest));
                    continue;
                }
                ...
                if (line.rfind("/w ", 0) == 0 || line.rfind("/whisper ", 0) == 0)
                {
                    ...
                    WhisperRequest w_req{};
                    w_req.header.packet_size = sizeof(WhisperRequest);
                    w_req.header.message_type = MessageType::WHISPER_REQUEST;
                    w_req.header.user_id = client->GetUserId();
                    w_req.room_id = created_room_id;
                    std::strncpy(w_req.target_username, target_user.c_str(), sizeof(w_req.target_username) - 1);
                    std::strncpy(w_req.message, msg_body.c_str(), sizeof(w_req.message) - 1);

                    client->Send(&w_req, sizeof(WhisperRequest));
                    continue;
                }

                ChatMessage chat_msg{};
                chat_msg.header.packet_size = sizeof(ChatMessage);
                chat_msg.header.message_type = MessageType::CHAT_MESSAGE;
                chat_msg.header.user_id = client->GetUserId();
                chat_msg.room_id = created_room_id;
                std::strncpy(chat_msg.message, line.c_str(), sizeof(chat_msg.message) - 1);

                client->Send(&chat_msg, sizeof(ChatMessage));
            }
        }
        else if (choice == 3)
        {
            ...
            if (!joined)
            {
                std::cout << "[System] Failed to enter room." << std::endl;
                continue;
            }
            ...
            while (client->IsConnected())
            {
                ...
                if (line.rfind("/kick ", 0) == 0)
                {
                    ...
                    kick_req.header.message_type = MessageType::KICK_USER_REQUEST;
                    kick_req.header.user_id = client->GetUserId();
                    kick_req.room_id = target_room_id;
                    kick_req.target_user_id = target_user_id;
                ...
                if (line.rfind("/pass ", 0) == 0)
                {
                    ...
                    pass_req.header.message_type = MessageType::TRANSFER_MASTER_REQUEST;
                    pass_req.header.user_id = client->GetUserId();
                    pass_req.room_id = target_room_id;
                    pass_req.target_user_id = target_user_id;
                ...
                chat_msg.header.message_type = MessageType::CHAT_MESSAGE;
                chat_msg.header.user_id = client->GetUserId();
                chat_msg.room_id = target_room_id;
                std::strncpy(chat_msg.message, line.c_str(), sizeof(chat_msg.message) - 1);
